...
class BacktestEngine {
    fun run(candles: List<Candle>, funding: List<FundingRate>, strategy: com.notash.cryptobacktester.strategy.Strategy, config: BacktestConfig): BacktestReport {
        ...
        for (i in candles.indices) {
            ...
            if (position == null && pendingSignal != null) {
                ...
                val filled = when (signal.side) { Side.LONG -> candle.low <= signal.entryPrice; Side.SHORT -> candle.high >= signal.entryPrice }
                if (filled) {
            ...
            if (currentPosition != null) {
                ...
                if (exit != null) {
                    ...
                    val reason = when {
                        currentPosition.side == Side.LONG && candle.low <= currentPosition.stopLoss -> "SL"
                        currentPosition.side == Side.LONG && candle.high >= currentPosition.takeProfit -> "TP"
                        currentPosition.side == Side.SHORT && candle.high >= currentPosition.stopLoss -> "SL"
        ...
        if (position != null) {
            val lastCandle = candles.last(); val finalPosition = position!!
            ...
            val fundingCost = calculateFunding(finalPosition, funding, lastCandle.timestamp, config)
    ...
    private fun calculatePositionSize(balance: Double, entry: Double, stop: Double, config: BacktestConfig): Double {
        val riskAmount = balance * config.riskPercent / 100.0; val stopDistance = abs(entry - stop)
        if (riskAmount <= 0.0 || stopDistance <= 0.0 || entry <= 0.0) return 0.0
        return minOf(riskAmount / stopDistance, (balance * config.leverage) / entry)
    }
    private fun calculatePnl(position: Position, exitPrice: Double): Double = when (position.side) { Side.LONG -> (exitPrice - position.entryPrice) * position.quantity; Side.SHORT -> (position.entryPrice - exitPrice) * position.quantity }
    private fun checkExit(position: Position, candle: Candle): Double? = when (position.side) { Side.LONG -> when { candle.low <= position.stopLoss -> position.stopLoss; candle.high >= position.takeProfit -> position.takeProfit; else -> null }; Side.SHORT -> when { candle.high >= position.stopLoss -> position.stopLoss; candle.low <= position.takeProfit -> position.takeProfit; else -> null } }
    private fun applyEntrySlippage(price: Double, side: Side, config: BacktestConfig): Double { val f = config.slippageBps / 10000.0; return when (side) { Side.LONG -> price * (1.0 + f); Side.SHORT -> price * (1.0 - f) } }
    private fun applyExitSlippage(price: Double, side: Side, config: BacktestConfig): Double { val f = config.slippageBps / 10000.0; return when (side) { Side.LONG -> price * (1.0 - f); Side.SHORT -> price * (1.0 + f) } }
    private fun calculateFunding(position: Position, funding: List<FundingRate>, exitTimestamp: Long, config: BacktestConfig): Double {
        if (!config.useFunding) return 0.0
        val applicable = funding.filter { it.timestamp > position.entryTime && it.timestamp <= exitTimestamp && it.rate != 0.0 }
        var total = 0.0
        for (rate in applicable) {
            val referencePrice = if (rate.markPrice > 0.0) rate.markPrice else position.entryPrice
            val payment = referencePrice * position.quantity * rate.rate
            total += when (position.side) { Side.LONG -> payment; Side.SHORT -> -payment }
        }
        return total
    }
}
