...
/** Stable interchange contract for AI-generated and imported strategies. */
    ...
    override fun generateSignal(
        ...
        return when {
            long -> {
                ...
                Signal(com.notash.cryptobacktester.core.Side.LONG, com.notash.cryptobacktester.core.OrderType.MARKET, c.close, sl, tp, "ALVEX robot long rule")
            }
            short -> {
                val sl = c.close * (1.0 + robot.parameters.stopLossPercent / 100.0)
                val tp = c.close * (1.0 - robot.parameters.takeProfitPercent / 100.0)
                Signal(com.notash.cryptobacktester.core.Side.SHORT, com.notash.cryptobacktester.core.OrderType.MARKET, c.close, sl, tp, "ALVEX robot short rule")
            }
            else -> null
        }
    }
