...
class AlvexIntelligenceCoordinatorTest {
    ...
    fun incompleteProvidersAreReportedAsGaps() {
        val report = AlvexIntelligenceCoordinator().analyze(
            AlvexIntelligenceInput(ProjectProfile(), ProductAnalysis(), DevelopmentActivity(), nowDate = "2026-09-01"),
    ...
    @Test
    fun confirmedInputsProduceAnAggregateReport() {
        val source = SourceRef("official", "https://example.com", "2026-09-01T00:00:00Z", SourceStatus.CONFIRMED)
        val report = AlvexIntelligenceCoordinator().analyze(
            AlvexIntelligenceInput(
                ProjectProfile(name = "Example", symbol = "EX", website = "https://example.com", source = source),
                ProductAnalysis(utility = "Payments", score = 80.0, confidence = 90.0, sources = listOf(source)),
                DevelopmentActivity(latestCommit = "abc", score = 75.0, sources = listOf(source)),
                roadmap = listOf(RoadmapMilestone("v1", status = MilestoneStatus.COMPLETED, source = source)),
                roadmapOfficial = true,
                team = TeamIntelligence(members = listOf(TeamMember("Founder", source = source)), anonymous = false, confidence = 80.0, sources = listOf(source)),
