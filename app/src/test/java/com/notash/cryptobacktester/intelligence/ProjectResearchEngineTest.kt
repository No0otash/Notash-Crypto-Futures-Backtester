...
class ProjectResearchEngineTest {
    ...
    fun missingResearchDataIsExplicit() {
        val summary = ProjectResearchEngine().summarize(
            ProjectProfile(), ProductAnalysis(), DevelopmentActivity(), "2026-08-30T00:00:00Z"
        )
    ...
    fun allocationTotalIsValidatedAndLargeUnlockFlagged() {
        val assessment = TokenomicsEngine().assess(
            ...
            listOf(
                Allocation("team", 20.0, unlock = "2027", confidence = 90.0),
                Allocation("community", 70.0, confidence = 90.0)
            )
        )
